...
def find_results_by_keywords(dataset, keywords):
    ...
    for index, row in dataset.iterrows():
        ...
        if matching_keywords:
            named_entities = extract_named_entities(row['summary'])
            ...
            search_result_id += 1
            matching_results.append((search_result_id, row['title'], row['issued'], matching_keywords, summary, score, named_entities))
...
def extract_named_entities(text):
    named_entities = set()  # Use a set to store unique named entities

    # Process the text with spaCy
    doc = nlp(text)

    for ent in doc.ents:
        entity_text = ent.text  # Get the entity text
        entity_label = ent.label_  # Get the entity label
        named_entities.add((entity_text, entity_label))  # Add the entity and label as a tuple to the set
